...
export class UsersController {
  static async listUsers(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const { users, meta } = await UsersService.listUsers({ page, limit, search, role, status });
  ...
  static async verifySalesOfficer(req: Request, res: Response, next: NextFunction) {
    try {
      const user = await UsersService.verifySalesOfficer(req.user!.id, req.params.id);
      return sendSuccess(res, user, 'Sales Officer verified and activated successfully.');
    } catch (error) {
      next(error);
    }
  }

  static async rejectSalesOfficer(req: Request, res: Response, next: NextFunction) {
    try {
